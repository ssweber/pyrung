...
- Bool feedback couplings (`Physical(on_delay=…, off_delay=…)` with `link=`) are now **dwell**: the synthesized feedback responds to a *sustained* command, so a command pulse shorter than `on_delay` no longer fabricates feedback (previously a transport-delay heap scheduled `Fb` on the rising edge and asserted it even if the command had already dropped). The harness lowers each bool coupling to a real on-delay/off-delay timer pair that the soft-exec runner scans as plant feedback *after* the program logic — so the dwell is bit-equal to a hand-written `TON`→`TOF`, `run_until(fold=True)` folds through long feedback delays in a handful of real scans, and (because the plant reads the scan's settled command) the synthesized feedback now commits the same scan its command is active rather than a scan later. The program reads that feedback the next scan exactly as before, so program behavior is unchanged; only the committed feedback timeline shifts one scan earlier (the more physical model). The feedback is plant-model scaffolding for `how()` planning and soft-exec testing — it is never emitted to a Click/CircuitPython controller or seen by the `prove` verifier, and on replay it is recomputed deterministically (not recorded).
