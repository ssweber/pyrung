...
- `prove()` / `reachable_states()` soundness — indirect block writes (`DS[DS[1] + 2]`) were invisible to both the PDG write-target analysis and the literal-write domain extractor, causing block elements to be excluded as `unwritten_internal` and their values to be untracked in BFS.
