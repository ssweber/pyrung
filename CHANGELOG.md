...
- **`prove()` BFS hot-path optimizations** — five micro-optimizations targeting profiled hot spots: (1) `_merge_values` short-circuits on identity (`a is b`) before equality; (2) `AbstractState.set` short-circuits on identity before equality; (3) `AbstractState.get` uses `in` check to avoid unconditional `base.get`; (4) `_is_numeric_literal` uses `type(value) in {int, float}` instead of double `isinstance`; (5) codegen emits per-type `_store_int`/`_store_dint`/`_store_word`/`_store_real`/`_store_bool` helpers instead of dispatching through the generic `_store_copy_value_to_type` string lookup. Net result: `__eq__` calls down 40% (9.6M → 5.6M), `isinstance` calls down 16% (5.6M → 4.7M).

### Fixes

- **`prove()` external timer preset default=0 blocks absorption** — both redundant accumulator absorption and threshold vector absorption now check whether an external preset/threshold tag's default already satisfies the comparison at initialization (e.g. `Acc >= 0` is trivially true when both default to 0). Previously both paths collapsed the accumulator out of the state space, hiding the trivially-crossed initial state and producing unsound `Proven` results.
...
- **`prove()` OTE inside dynamic `ForLoop` no longer misclassified as combinational** — `out()` nested inside a `ForLoop(count_tag)` where the count tag can be 0 at runtime is now correctly classified as stateful. When count=0 the ForLoop body is skipped and the tag retains its previous-scan value, making it stateful rather than combinational. Also adds ForLoop count ND inputs to the always-live set so the BFS live-input pruner never masks them.
- **`prove()` receive dest not absorbed by comparison absorption** — `receive()` destination tags are no longer silently consumed by comparison-only absorption before reaching the ND input classification path. Previously a receive dest compared downstream (e.g. `value >= 500`) was absorbed into a threshold vector, removing it from the state space entirely and causing `Proven(states_explored=1)` with no values explored.
- **Oneshot `out()` writes False after firing instead of retaining entry** — `out(tag, oneshot=True)` now matches Click spec: the output is True for exactly one scan on the rising edge, then False while the rung stays enabled. Previously the instruction skipped the write after firing, silently retaining the tag's entry value (always True). Both interpreted and compiled paths are fixed.
- **`prove()` abstract elision handles oneshot `out()` precisely** — the abstract provenance analyzer now treats oneshot `out()` the same as regular `out()` instead of bailing to `_UNKNOWN_VALUE`. Since the corrected instruction always writes (True or False), the tag's entry value never leaks through, making the elision rule sound. The concrete elider also excludes `_oneshot:` memory keys from warm-memory sensitivity checks, which tested memory independence rather than the entry independence that elision requires.
...
- **`prove()` backward domain propagation through data paths** — ND input domains now include comparison boundary values from downstream tags fed through `copy()`, `fill()`, `blockcopy()`, or invertible `calc()` expressions (supports `+`, `-`, `*`, and unary `+`/`-`). Previously only `copy()` and `calc(source ± k, dest)` were supported; `*`, `fill()`, and `blockcopy()` writes were silently skipped, under-approximating source domains. Transitive chains (e.g. `copy(level, stored)` then `calc(stored * 2, shifted)`) now propagate boundaries through multiple hops. Non-invertible expressions (like `%`) trigger a fallback: sources with declared `min`/`max`/`choices` widen to their full declared domain; truly unbounded sources return `Intractable`.
- **`prove()` backward propagation through pointer-indirect writes** — `copy(source, block[ptr])`, `fill(value, block.select(ptr, ptr))`, and `blockcopy(src, block.select(ptr, ...))` now propagate comparison boundaries from concrete block elements back to the source tag. Previously indirect destinations were silently skipped (no reverse edge built, no blocker raised). The expansion uses the pointer's `min`/`max`/`choices` to tighten the set of possible targets; `IndirectBlockRange` and `IndirectExprRef` fall back to full block bounds.
- **`prove()` external preset absorption guard** — `_find_redundant_acc_absorptions` no longer absorbs timer/counter accumulators when the preset tag is `external` and the accumulator's default already meets or exceeds the preset default. In that scenario the `Done` comparison is trivially true at initialization, so absorbing the Acc would collapse reachable states.
- **`prove()` threshold absorption init-crossed guard** — `_find_threshold_absorptions` no longer absorbs a progress accumulator when any threshold atom references an `external` tag whose default is already crossed by the accumulator's default value. Previously the threshold vector was accepted even when the initial state satisfied it, collapsing states that should remain explicit.
...
- **`__lock__` `joint` key — joint multi-flip input groups** — `__lock__` now accepts a `joint` dict mapping group names to lists of tag names. Single-flip BFS normally changes one input at a time; grouped inputs additionally generate multi-flip combinations. Use sparingly — logic that only works when inputs change in the same scan cycle is usually a design problem. Passed through as `joint_inputs=` on `reachable_states()` and `prove()`.
