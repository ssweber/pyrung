...
- `plc.how(condition)` no longer requires a prior `plc.explore()` call — it runs a snapshot-seeded BFS directly from the current PLC state, using heuristic domain seeding to resolve programs that would otherwise be `Intractable`. When an explored transition graph exists, how() still delegates to the graph-based shortest-path search for full feature support (`avoid=`, `minimize="changes"`).
- `how()` now decomposes multi-step targets into waypoints — stateful tags that must change value along the path. Each waypoint gets a cone-scoped mini-BFS instead of one monolithic search, with generator-based backtracking when an intermediate solution leads to a dead end. Falls back to undecomposed BFS transparently when decomposition isn't applicable.
- Heuristic domain seeding now handles cross-correlated Real inputs: when two ND tags appear in the same comparison (e.g. `pv >= upperBand`), each side's domain is expanded to include values on both sides of the partner's values. Non-default snapshot values also seed neighbor probes for bisection, so `how()` from a live PLC state explores around the operating point instead of only at type boundaries.
