...
- `plc.how(condition)` runs a snapshot-seeded BFS directly from the current PLC state, with full `avoid=` support. Heuristic domain seeding resolves programs that would otherwise be `Intractable`.
