...
- Bool feedback couplings (`Physical(on_delay=…, off_delay=…)` with `link=`) are now **dwell**: the synthesized feedback responds to a *sustained* command, so a command pulse shorter than `on_delay` no longer fabricates feedback (previously a transport-delay heap scheduled `Fb` on the rising edge and asserted it even if the command had already dropped). The harness now lowers each bool coupling to a real on-delay/off-delay timer pair ticked before each scan, so the dwell is bit-equal to a hand-written `TON`→`TOF`, `run_until(fold=True)` folds through long feedback delays in a handful of real scans, and the timing of sustained transitions is unchanged.
