...
- **Codegen API cleanup** — `TagMap.to_ladder(program)` removed; use free function `to_ladder(program, tag_map)`. `csv_to_pyrung()` renamed to `to_pyrung()` (alias kept). Both exported from `pyrung.click`.
...
- **Click ladder CSV export** — `to_ladder(program, tag_map)` generates deterministic 33-column CSV files importable into Click programming software. Supports AND/OR expansion, branch continuation rows, forloop lowering, subroutine splitting, and `LadderBundle.write()` for multi-file output.
- **In-memory round-trip** — `to_pyrung(bundle)` accepts a `LadderBundle` directly, enabling program → ladder → Python source round-trip without writing CSV to disk. Also accepts file paths (replaces `csv_to_pyrung`).
...
- **`TagMap.from_hardware()`** — translates a hardware-addressed dict (e.g. from `pyclickplc.read_plc_data`) to logical tag names, so a PLC data dump can initialize a runner in three lines.
