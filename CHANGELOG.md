...
- Bool feedback couplings (`Physical(on_delay=…, off_delay=…)` with `link=`) are now **dwell**: the synthesized feedback responds to a *sustained* command, so a command pulse shorter than `on_delay` no longer fabricates feedback (previously a transport-delay heap scheduled `Fb` on the rising edge and asserted it even if the command had already dropped). The harness lowers each bool coupling to a real on-delay/off-delay timer pair, and the soft-exec runner scans it as a **plant at the top of the scan — the input-read phase**: the synthesized feedback is an *input* that reflects the *previous* scan's committed command, so it lags the command by one scan (with `on_delay == 0` the program sees the feedback the very next scan; with a longer delay the response is the dwell plus that one input-read scan). This is the physical model — a sensor cannot respond to an output before that output is written. The dwell is bit-equal to a hand-written `TON`→`TOF`, and `run_until(fold=True)` folds through long feedback delays in a handful of real scans. The feedback is plant-model scaffolding for `how()` planning and soft-exec testing — it is never emitted to a Click/CircuitPython controller or seen by the `prove` verifier, and on replay it is recomputed deterministically (not recorded).
...
- `run_until()` / `run_for()` time-folding now also collapses a heartbeat clock gating a *saturated* accumulator comparison (e.g. `with Rung(rise(clock_1s), Tmr.Acc > 100): ...` while the timer ramps to a far preset): once the comparison has settled past its threshold the gated logic recomputes an identical result at every edge, so the clock's edges are skipped instead of capping the fold at the clock period — a 30-second wait collapses from ~60 real steps to a handful while staying bit-equal to scan-by-scan.
- `run_until()` / `run_for()` time-folding no longer misevaluates `rise()` / `fall()` on a system clock when a fold step crosses clock edges: an edge-gated rung that pulses (e.g. `with Rung(rise(clock_1s), Tmr.Acc > 100): out(Beat)`) now ends bit-equal to scan-by-scan. Three faults are fixed — a resolved-on-read clock's edge-detection state is now computed at the true previous scan across a folded step (it was captured at the step's start, leaving the next scan comparing against a stale value); a clock is marked skippable only after a full period is observed inert (a rung gated on the rising edge is inert at the intervening fall, which previously marked the whole clock skippable and dropped the next rise); and the fold now lands strictly before an edge that falls on an exact integer of scans instead of straddling it.
