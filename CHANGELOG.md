...
- **PILOT's `via=` route selector was removed.** `how()` still reports its
  deterministic route and alternatives; use `avoid=` to exclude the reported
  route and let PILOT select another current-world alternative. Route receipts
  now use the plain route name in `RouteTaken.label` (without a `via ` prefix),
  rename `RoutePivot.via_hint` to `RoutePivot.avoid_hint`, and remove
  `RouteAlt.via_hint`.
...
- **PILOT backward tracing is substantially faster on large generated programs.** Rung-local static writer facts, bounded series/parallel expression reuse, early singleton-rejection checks, and snapshot-local writer-ranking receipts cut the instrumented cold-boot `how(y_BurnerLoop)` trace-tree cost from 29.3 s to about 9.5 s and total CPU from 60.7 s to about 38.4 s while preserving its 98-event route and scan-2,122 landing.

- **`stop` cancels a running analysis.** A long `how` can now be interrupted: `pyrung live stop` — or Ctrl+C during `pyrung live how …`, which sends the same request — asks the session to abandon the search and return `how cancelled`, leaving forces, scan position, and the rest of the session intact. The live server answers `stop` ahead of the adapter's state lock and serves each connection on its own thread, so the request reaches the session while the command being cancelled still holds that lock. `how` polls for the stop wherever it reports progress, so a cancel lands within one planner phase — usually milliseconds, though a phase that computes for seconds without emitting progress runs to its end first.
- **New `STEP_NO_ESCAPE` validation rule — a step that can hang forever.** `logic.validate()` now flags a step whose only advance needs something the program cannot supply (a feedback contact, an analog threshold, an operator's button) when no escape can fire unaided — the machine sits there looking fine. It recognizes step machines written as `calc(Step + 1, Step)` or `copy(2, Step)`, gated on a level or `rise()`, and it names the escapes that *look* like coverage and aren't: a timeout guarding the wrong step, or one gated on a register nothing sets. The survey behind it is `query.wait_edges_without_escape()` (static, needs no scan history). It reports a design decision and never edits the program, and stays silent on guards it cannot read statically rather than inventing a verdict.
- **The console grammar is published as data (`pyrung.dap.grammar`).** `command_grammar()` returns a `CommandGrammar` per console verb, whose `Slot`s say what each argument accepts (`tag`, `expression`, `choices`, `flag`, …), whether it repeats, what separates the repeats (`how A, B` — comma), and which literal keyword introduces it (`avoid`). Editors and REPL front-ends that complete console input should read this instead of parsing the human-facing `usage=` string, which cannot express any of those facts. The `usage=` string remains the source of truth for 35 of the 36 commands — their slots are *derived* from it — so adding a command needs no extra declaration; only `how` declares its slots explicitly via `register(slots=…)`, because comma-separated targets and keyword clauses are not recoverable from prose. The derivation is heuristic, so it is pinned by tests here (including a guard that a usage naming `<tag>`/`<expression>` must yield a completable slot) rather than failing silently in a downstream tool.
- **The pilot rides the exact deep cause chain.** The pilot consumes fired-rung triggers and recursively explained enablers instead of re-walking broad static writer sets. Causal-primacy ranking uses the transition spine, so a steady enabler cannot outrank the trigger that made it newly relevant.
- **`how()` regression investigation names the newly conductive input, not a bystander.** When a coast aborts, investigation derives correctives from the exact fired rung: transitions stay triggers, while steady contacts that only became relevant because the rung fired remain enablers. Pilot-owned tags are not automatically exempted from incident evidence. Exact cuts through requested progress remain visible as hypotheses; after their short replay, a direct or indirect contradiction of the incident bearing/checkpoint frontier is recorded as a proven `self-defeat` rejection instead of being silently pruned.
...
- **Recorded `cause()` and `effect()` preserve exact instruction-level execution occurrences.** Repeated writes to one tag, parent/subroutine interleaving, and reads through frozen `continued()` views now resolve to the actual definition observed instead of an inferred first, final, or same-valued write.
- **`cause(deep=True)` recursively explains exact enablers.** An enabler that moved earlier is followed to its establishing transition; a steady program-owned enabler is followed only through a writer observed supplying that value at the incident. Non-firing static writers are not swept into the chain. Terminals are classified in `chain.roots`; `cause(tag, deep=False)` restores the shallow trigger-only walk.
...
- **`CMP_*` comparison-semantics validators.** `validate()` flags six comparison mistakes: `CMP_EQ_ON_MONOTONE` (error — `==`/`!=` against a self-advancing accumulator that can step over the value between scans; suggests `>=`/`<=` or the Done bit, exempting the edge-safe `== 0`/`!= 0` floor), `CMP_TRUE_AT_RESET` (warning — an ordered comparison that is true at `Acc = 0` and false at the crossing, the inverted completion check that pulses on every state entry), `CMP_STATIC_ON_LEFT` (the operand-order convention "moving value on the left, expectation on the right"), `CMP_OPERAND_STAYS_ZERO` (warning — a numeric comparison operand has an implicit zero start and nothing sets it), `CMP_PRESET_STAYS_ZERO` (warning — a timer/counter preset likewise stays zero, making completion immediate), and `CMP_STEPPER_VALUE_NOT_SET` (warning — a discrete stepping tag (state-machine/step/stage logic) is tested with `==` against a value absent from every understood direct-copy, copy-chain, and constant indirect-table producer). The zero rules leave explicit defaults, first-scan writes, external or physical inputs, read-only constants, and intentional literal-zero presets alone; the stepper rule likewise stays quiet for `!=`, ordered comparisons, and any unresolved or externally writable producer path. The operand-order rule grades itself by confidence: an accumulator-anchored issue is provable and reported as a **warning** (or escalates into `CMP_TRUE_AT_RESET`), while two ordinary tags — where a live measurement and a threshold are indistinguishable — surface as an **advisory** that stays out of the `errors()`/`warnings()` gate. Select the whole bucket with `validate(select={"CMP"})`.
...
- **`how()` rewritten.** The v0.10.0 planner pre-computed a route, then tried to execute it. The new engine steers toward the goal one step at a time — read the program's causal structure, act, verify what moved, adjust. It returns a fork-backed `Plan` whose scan log *is* the replayable proof. Building it drove enhancements across `cause()`, `upstream_slice`, `ProgramGraph`, and the crossing registry — `pilot/` is pyrung's largest consumer of the analysis stack.
  - **Route selection.** Multi-path Bool targets take a deterministic default route; exclude it with `avoid=` to let PILOT select another current-world alternative. DAP: `how State == HELD avoid State == FAULTED`.
...
- **`how(avoid=…)` excludes routes, operator actions, and observed scan states.** `avoid X` now means "do not take a path that depends on X" across three gates: the route gate prunes routes that force X, a new action gate rejects a candidate (or a corrective/prerequisite hold) whose action would make X true *before* it is pulsed — so `avoid=C_Complete` no longer presses `C_Complete` even though the momentary command settles back to rest — and the scan gate vetoes transient exposure too. Condition-like members ride folded trial coasts as named bumps, preventing a skipped two-scan wink; opaque Python callables are checked only at endpoints, retained real snapshots, and kernel scans a fold executes because no readable condition can constrain skipped logical scans. `rise()`/`fall()` transition predicates are rejected by `avoid=`. When every path is excluded the unreachable `Path` names the violated avoid condition(s). **Tuple/list semantics changed:** `avoid=(A, B)` / `avoid=[A, B]` is now a **union of exclusions** (each avoided independently) rather than a conjunction; express a composite prohibition explicitly as `avoid=And(A, B)`. The DAP `how … avoid A, B` console command follows the same union semantics.
...
- A corrective hold installed by `how()` no longer renders a repeated disjunct such as `Or(State == 4, State == 3, State == 3)`. The hold's scope is assembled from an incident's source, exposure, and landing roles, and two roles routinely name the same state; the union now collapses structurally identical terms.
...
- Click codegen topology validation now names the main program or subroutine plus its 1-indexed rung, permits valid conditions merged across outputs, and still rejects genuinely disconnected conditions or dropped output instructions.
- Click codegen now renders rung comments with safe Python literals, so quotes, backslashes, and multiline text cannot produce invalid generated source.
