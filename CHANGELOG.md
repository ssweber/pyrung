...
- **Stuck-bit static validator** — `validate_stuck_bits(program)` detects latch/reset imbalance at the program level: `CORE_STUCK_HIGH` (latched, never reset) and `CORE_STUCK_LOW` (reset, never latched). Covers subroutine boundaries.
