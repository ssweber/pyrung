...
## FIRST CASUALTY (decided 2026-07-14): the zoom trend-noise false-confirm
...
- A channel zoom (let-run `Sts_StateCurrent: 3->6`) ran its full `_ZOOM_BUDGET=10_000`
  with the channel FROZEN at 3 (`zoom_accepted outcome=confirmed req=6 actual=3
...
- The false CONFIRMED starves a **fully functional** escalation: when the same blind
  zoom is honestly rejected (shallow-tree drives, e.g. `how(Sts_State_Execute==True)`),
...
- Related non-blocking residuals for the same rework: attach coupling drivers when
  ...
  `_ZOOM_BUDGET=10_000` vs `_PROVISIONAL_SCAN_BUDGET=2_000` asymmetry (a zoom can
