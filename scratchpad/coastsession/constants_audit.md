...
- A channel zoom (let-run `Sts_StateCurrent: 3->6`) ran its full `_COAST_BUDGET=10_000`
...
- Related non-blocking residuals for the same rework: attach coupling drivers when
  ...
  `_COAST_BUDGET=10_000` vs `_PROVISIONAL_SCAN_BUDGET=2_000` asymmetry (a zoom can
