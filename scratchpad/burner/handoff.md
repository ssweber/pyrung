...
history cleanliness — and `test_pilot_credential.py` / the flipped
`test_pilot_detour_progress.py` for the gates).

- `pilot/credential.py` — the target-relative progress credential.  Two
  ...
  *stepper* (`Internal__Step`: +1 affine advances with discrete OR
  ...
  erasers, enabling channel values resolved one hop through the
  `sm_MapVal2State` alias Bools).
- `pilot/detour.py` — landing classification: a **stopover** needs a clean
  forward road on the compass value graph — no resident eraser (a literal
  ...
  already-committed press in the same channel context — the road from
  ABORTED re-requires Clear/Reset/Start; the road from HELD requires
  `C_Unhold`, never discharged).  Everything else → regression → the old
  investigate-and-revert, unchanged.
- **The detour loan** (progress.py): a stopover is provisional.  Adopt the
  settled landing, keep the pre-departure checkpoint as bailout, skip
  investigation; settle at corridor rejoin by credential compare — advanced
  → promote to a checkpoint (trend baseline resets), anything else → revert
  to bailout + remember the failed signature (the re-ejection then
  classifies regression and investigation gets a tight fresh window).
- **Verify gates consult the credential**: SPIN / CYCLE / LATERAL accept a
...
- **Budget charges searching, never productive waiting** (`dwell_scans` on
  ...
  reached its channel target / advanced the credential / landed somewhere
...
```text
detour: S_StateCurrent 6->9  (102 settle scans): regression — every forward road
        crosses a credential eraser or resurrects a discharged obligation
        -> investigation runs, rotate/temp correctives earned as before
detour: S_StateCurrent 6->11 (102 settle scans): stopover — clean forward road
        11 -> 12 -> 6 (no eraser, no resurrected obligation)
        -> DETOUR LOAN, no investigation round, march preserved
```

## Frontier: driving the HELD handshake (the loan never settles)
...
- The 105→107 advance needs a **door cycle** (`x_DoorClosed` False→True at
  ...
  **earned corrective hold**.  The hold-release machinery
  (`hold_defeats_needed` / self-defeat-release) only runs in the
  investigate-and-revert arm, which the stopover rightly skips.  The loan
  seam needs the same release check: a held input that pins a frontier need
  away from its needed value at the *new* state should release (and the
  trace re-propose it as a pulse).
- After the door cycle, `C_Unhold` is the one legal button (the clean road's
...
uv run python scratchpad/burner/probe_credential_cut.py           # cut recognition on burner + knock
...
uv run pytest tests/core/analysis/test_pilot_credential.py tests/core/analysis/test_pilot_detour_progress.py -q
```
...
- The HELD loan *settles as promoted* (credential advanced at the Execute
  rejoin) — watch for `detour_promoted` in the event stream.
