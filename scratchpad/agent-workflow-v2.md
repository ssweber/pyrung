...
- Agent talks to pyrung-live for simulation; clicknick-cli for annotations/rung edits
...
Coverage tools also fit here: `query.cold_rungs()`, `query.stranded_bits()`, `query.hot_rungs()`. These survey recorded scan history without needing annotations. **Note:** these are Python API only — not yet exposed as `pyrung live` commands. The agent can't use them through the CLI today.
...
└── .claude/
    ├── settings.json      # Permissions for clicknick-cli, pyrung-live
    └── skills/
...
- Per-subroutine: name, rung count, one-line description of purpose
...
Any way to automatically generate this from the program graph would be helpful — the emitter already has the structure, so rung counts and tag roles are free. For one-line descriptions: Click convention often puts a comment on the first rung of a subroutine describing its purpose — if present, use that. Fall back to heuristics (e.g. "contains alarms" if the sub writes alarm tags, "step N handler" if gated by a step comparison) when no comment exists.
...
## Read and edit program structure (via clicknick-cli)
    ...
    clicknick-cli rung list                  # summary per rung
    clicknick-cli rung list init             # subroutine rungs
    clicknick-cli rung preview --select r3   # before/after diff
    clicknick-cli rung apply                 # convert edits to ladder CSVs
...
2. `clicknick-cli rung preview <file>` — opens preview window with diff and Copy button (requires apply first for the Copy button to be active)
...
6. `clicknick-cli rung apply` to prepare ladder CSVs
7. Engineer reviews via `clicknick-cli rung preview`, applies in Click, saves
...
7. `clicknick-cli rung apply` to prepare output
...
### Rung-level workflow (via clicknick-cli)

```
clicknick-cli rung list                  # see program structure
# Agent edits pyrung source files...
clicknick-cli rung preview --select r3   # diff showing what changed
clicknick-cli rung apply                 # convert to ladder CSVs in csv_output/
```
...
│                     │     │  force/patch        │     │              │
│  LiveServer─────────┼─────┼──clicknick-cli────┼─────┼──annotations │
│  (tag/rung cmds)    │     │                    │     │  rung edits  │
└─────────────────────┘     └────────────────────┘     └──────────────┘
```

- **pyrung-live** — agent uses for simulation: step, patch, force, why, how, prove, cause, effect
- **clicknick-cli** — agent uses for data: tag annotations, rung list/preview/apply, get/set fields
...
This means the agent can run the full annotation loop autonomously — attempt `always()`/`never()`, read blocker, annotate via clicknick-cli, retry, next blocker, annotate, retry, success — without asking permission at each step. The engineer reviews the batch at the end: "The agent added six annotations to close the state space. Review in the address editor." The engineer glances at the diff, corrects the one wrong sensor range, saves. Ten seconds.

### Commands (implemented)

**Flags:**
```
clicknick-cli tag set-flag <tag> <flag>      # readonly, external, final, public, lock
clicknick-cli tag clear-flag <tag> <flag>
```

**Value constraints:**
```
clicknick-cli tag set-choices <tag> <Label:val> ...   # or Bool
clicknick-cli tag set-range <tag> <min> <max>
clicknick-cli tag clear-constraints <tag>
clicknick-cli tag set-uom <tag> <unit>
clicknick-cli tag clear-uom <tag>
```

**Physical devices:**
```
clicknick-cli tag set-physical <tag> <name> [--on-delay D] [--off-delay D] [--profile P] [--system S]
clicknick-cli tag set-link <tag> <link>
clicknick-cli tag clear-physical <tag>
```

**Queries:**
```
clicknick-cli tag show <tag>
```

**Rung operations:**
```
clicknick-cli rung list [file]
clicknick-cli rung preview [file] [--select r3,r7]
clicknick-cli rung apply [file]
```

**Note:** `rung preview` requires a subroutine name — bare `rung preview` with no argument reports no changes. Ideally, `rung preview` with no argument should list which files have changes so the agent doesn't have to guess.

**Basic operations:**
```
clicknick-cli ping
clicknick-cli prompt-save
clicknick-cli get <tag-or-addr>
clicknick-cli set <tag-or-addr> <field> <value>
```

All tag/rung identifiers resolve by pyrung tag name first, Click display address (DS1) as fallback.

Both `pyrung live` and `clicknick-cli` support `;` chaining — batch commands in one call:

```
clicknick-cli "tag set-range LevelPV 0 100; tag set-flag LevelPV external; tag show LevelPV"
...
When the agent pushes annotations through clicknick-cli, they flow through a two-step pipeline — analogous to commit + push in git.
...
**`clicknick-cli prompt-save`** — not yet implemented. The agent should be able to trigger a save dialog in ClickNick when it has pending changes (annotations, rung edits). Instead of explaining the two-step save process in chat every time, `prompt-save` pops a dialog in the GUI. The engineer reviews and saves from there.
...
- **`clicknick-cli ping` returns `project_stale: true`** — the agent can query this programmatically instead of hoping the engineer saved.
...
3. Agent batch-annotates via clicknick-cli (`tag set-choices`, `tag set-range`, etc.)
...
6. Agent checks `clicknick-cli ping` for `project_stale: false`, retries `always()`
...
- Available tools and their commands (clicknick-cli, pyrung-live)
...
## Appendix: Fill Station Walkthrough (June 2026)

A real session against a Click fill station program, exercising the full workflow. Included as a concrete example of what the escalation looks like in practice.

### Setup

Connected to the running ClickNick session via `clicknick-cli ping`, which returned the generated pyrung project path. Read `tags.py` (19 inputs, 6 timers, 8 Real tags), `main.py` (16-rung step machine), and 7 subroutines. Program is a fill station: off → waiting → filling → repeat, with level monitoring, alarms, stacklight, and a manual override.

### Review (Levels 1–2)

- `dataview i:` / `t:` / `p:` — mapped the I/O boundary (19 inputs, 18 terminals, 27 pivots)
- `simplified fill_solv_nc` — resolved the solenoid's logic chain back to root inputs, revealing XOR logic between auto fill and manual override
- `upstream fill_solv_nc` — showed 37 tags feeding the solenoid, spanning all subroutines

### Search (Level 4)

- `how fill_stepNumber == 3` — "turn on the machine and wait": `HMI_on=True`, 2 wait scans
- `how fill_stepNumber == 5` — initially unreachable (domain seeding couldn't handle the inverted analog conversion `pv_LevelHt = 100 - systemLevel_opt2011`). After fixing the domain seeder: 5 steps, 2 input changes
- `how fill_solv_nc` — found the shortcut path through `HMI_fill` (manual override)
- `how fill_solv_nc avoid HMI_fill` — found the full operational path through the fill sequence

### Bug 1: XOR on solenoid (found via simulation)

Simulated both auto fill and manual override active simultaneously:
```
patch HMI_on true; patch HMI_fill true; patch systemLevel_opt2011 101; step 10
get fill_solv_nc  →  False
```

Solenoid shut off with no alarm. Root cause: `Or(And(_fill, ~HMI_fill), And(HMI_fill, ~_fill))` — XOR logic means both sources cancel out. Fix: changed to `Or(_fill, HMI_fill)`. Verified with `simplified`: now reads `Or(fill_stepNumber == 5, HMI_fill), ~alarm`.

### Bug 2: Scan-ordering gap (found by prover)

After annotating 6 tags via `clicknick-cli` (choices on `fill_stepNumber`, ranges on 5 Real tags), ran formal verification:
```
prove never fill_solv_nc, alarm  →  Counterexample found
```

Replayed the counterexample in simulation — confirmed `fill_solv_nc = True AND alarm = True` simultaneously. Root cause: `call(io)` ran at rung 11, `call(error)` at rung 16. The solenoid saw the previous scan's alarm value for one scan cycle. Fix: moved `call(error)` before `call(io)`.

```
prove never fill_solv_nc, alarm  →  Proven (7,149 states explored)
```

### Round-trip

Both fixes applied back to Click via `clicknick-cli rung apply` / `rung preview` (copy-paste from the preview window). Saved in Click, ScrWatcher regenerated the pyrung project, re-proved against regenerated source — still proven.

### Workflow improvements discovered during the session

- **`how()` output noise** — default-value inputs cluttered the output. Fixed: filter tags at their default values from step 1 rendering.
- **`how()` domain seeding** — inverted analog conversions (`100 - raw`) tripped up the behavioral bisection. Fixed in the domain seeder.
- **`rung preview` needs a file argument** — bare `rung preview` reports no changes. Ideally it should list which files have changes.
- **`prompt-save` command** — added to `clicknick-cli` so the agent can signal the engineer to review and save, instead of explaining the two-step save process in chat.
- **`ping` folded `info`** — one command returns connection status, store size, project path, and sync state.
- **`clicknick-cli` flattened** — removed the redundant `live` subcommand, added `help`.
- **`how` avoid syntax** — doc had `avoid=expr`, actual CLI syntax is `how expr avoid expr`.
