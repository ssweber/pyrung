...
Returns an empty string when the program needs no runtime helpers.
"""
...
def _render_runtime(ctx: CodegenContext) -> str:
    """Return the pyrung_rt.py source, or ``""`` if nothing is needed."""
    helper_names = _needed_helpers(ctx)
    has_modbus_server = ctx.modbus_server is not None
    has_modbus_client = ctx.modbus_client is not None

    if not helper_names and not has_modbus_server and not has_modbus_client:
        return ""
    ...
    if helper_names:
        # struct is used by several helpers
        if helper_names & {
            "_int_to_float_bits",
            "_float_to_int_bits",
            "_parse_pack_text_value",
        }:
            imports_needed.add("struct")
        if helper_names & {
        ...
        if helper_names & {"_parse_pack_text_value"}:
            imports_needed.add("re")

    if has_modbus_server or has_modbus_client:
        imports_needed.add("struct")
    if has_modbus_client:
        imports_needed.add("time")
    ...
    # -- Modbus server registry slots -----------------------------------------
    if has_modbus_server:
        lines.extend(
            [
    ...
    # -- Pure helpers ---------------------------------------------------------
    if helper_names:
        lines.extend(_render_helper_defs(helper_names))

    # -- Modbus protocol handler (server) -------------------------------------
    if has_modbus_server:
        from pyrung.circuitpy.codegen.render_modbus import _render_modbus_protocol

        lines.extend(_render_modbus_protocol(ctx))
        lines.extend(_render_modbus_server_service(ctx))

    # -- Modbus client infrastructure -----------------------------------------
    if has_modbus_client:
        lines.extend(_render_modbus_client_runtime(ctx))
...
def _render_modbus_server_service(ctx: CodegenContext) -> list[str]:
    """Render ``service_modbus_server()`` for the runtime module."""
...
def _render_modbus_client_runtime(ctx: CodegenContext) -> list[str]:
    """Render Modbus client infrastructure for the runtime module."""
