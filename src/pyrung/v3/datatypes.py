...
class IntType(DataTypeDefinition):
    ...
    def validate(self, value: Any) -> int:
        ...
        if not (self.MIN_VAL <= int_value <= self.MAX_VAL):
            raise ValueError(f"INT value must be between {self.MIN_VAL} and {self.MAX_VAL}, got {value}")
        return int_value
...
class Int2Type(DataTypeDefinition):
    ...
    def validate(self, value: Any) -> int:
        ...
        int_value = int(value)
        if not (self.MIN_VAL <= int_value <= self.MAX_VAL):
            raise ValueError(f"INT2 value must be between {self.MIN_VAL} and {self.MAX_VAL}, got {value}")
        return int_value
