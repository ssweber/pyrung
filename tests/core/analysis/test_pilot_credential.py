"""Unit gates for the target-relative progress credential (pilot/credential.py)."""
...
def _cut_for(logic, plc, target_tag, channel_tags=frozenset()):
...
def test_knock_count_is_an_ordinal_component() -> None:
    """The threshold-absorbed knock counter joins the cut as an ordinal.

    The search key masks ``Knock_Count`` behind ``count < 3``; the credential
    carries its raw value so ``(AtDoor, 1) -> (AtDoor, 2)`` reads as an earn.
    """
    logic, _knock, channel, count, *_ = _knock_three_times_program()
    plc = PLC(logic, dt=0.010)
    plc.step()
    cut = _cut_for(logic, plc, channel.name)

    kinds = {c.tag: c.kind for c in cut.components}
    assert kinds.get(count.name) == "ordinal"
    assert cut.ordinal_advanced({count.name: 1}, {count.name: 2})
    assert not cut.ordinal_advanced({count.name: 2}, {count.name: 2})
    assert not cut.ordinal_advanced({count.name: 2}, {count.name: 0})


def _step_chain_program():
    """A discrete stepper with a reset eraser — the recipe-coordinate shape.
    ...
    ``Resetting`` is written under ``Mode == 15`` so the eraser's enabling
...
def test_step_chain_stepper_with_alias_resolved_eraser() -> None:
    logic, Step, Mode, Done = _step_chain_program()
    plc = PLC(logic, dt=0.010)
    plc.step()
    cut = _cut_for(logic, plc, Done.name, channel_tags=frozenset({Mode.name}))

    by_tag = {c.tag: c for c in cut.components}
    assert Step.name in by_tag, [c.tag for c in cut.components]
    component = by_tag[Step.name]
    assert component.kind == "stepper"
    assert component.direction == 1

    resolved = [e for e in component.erasers if not e.init_only]
    assert resolved, "the Resetting-gated load must be recorded as an eraser"
    eraser = resolved[0]
    assert eraser.resolved
    assert eraser.value == 1
    assert eraser.channel_tag == Mode.name
    assert eraser.enabling_channel_values == (15,)

    # Anchor-relative verdicts: ahead = advanced, equal = preserved,
    # a reset landing = behind (work destroyed).
    assert cut.compare({Step.name: 2}, {Step.name: 3}) == "advanced"
    assert cut.compare({Step.name: 2}, {Step.name: 2}) == "preserved"
    assert cut.compare({Step.name: 2}, {Step.name: 1}) == "behind"


def test_level_driven_counter_stays_out_of_the_cut() -> None:
    ...
    assert Count.name not in {c.tag for c in cut.components}
