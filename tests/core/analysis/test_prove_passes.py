...
class TestExplanation:
    ...
    def test_explain_classifications(self):
        ...
        assert any(d.kind == "classification" and d.outcome == "nondeterministic" for d in button_entry.decisions)
    ...
    def test_explain_exclusion_readonly(self):
        ver = Int("Version", readonly=True, default=1)
        out_tag = Bool("Out")
        button = Bool("Button", external=True)
        with Program() as logic:
            with Rung(button):
    ...
    def test_explain_redundant_absorption(self):
        ...
        if acc_entry is not None:
            has_absorption = any(
                d.kind in ("absorption", "exclusion") for d in acc_entry.decisions
            )
            assert has_absorption

    def test_explain_threshold_absorption_blocked(self):
        from pyrung.core.analysis.prove.absorb import _ThresholdBlocker
