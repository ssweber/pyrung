"""Adversarial BFS soundness tests for the prove() model checker.

Each test class targets a specific potential unsoundness gap identified
in an external audit of the BFS architecture.
"""
...
# ===================================================================
# Gap 1: Hidden-event acceleration skips intermediate accumulator values
# ===================================================================


class TestGap1IntermediateAccumulatorValues:
    """Timer/counter fast-forward must not skip intermediate-value-dependent logic.

    If a rung tests EQU(T1.ACC, 50) and the timer preset is 100, the
    intermediate state ACC=50 must be explored.  The threshold absorption
    decomposes eq/ne into two boundary atoms so the BFS distinguishes
    {<50, ==50, >50}.
    """
...
# ===================================================================
# Gap 2: Nondeterministic numeric inputs are under-enumerated
# ===================================================================


class TestGap2NumericInputEnumeration:
    """Unbounded numeric external inputs must not be silently under-enumerated.

    External Int/Dint with no metadata must either produce Intractable
    or be handled via boundary partitioning from comparison literals.
    """

    def test_unbounded_int_inputs_are_intractable(self):
        """Two unconstrained external Ints — should be Intractable, not silently truncated."""
    ...
    def test_single_unconstrained_int_is_intractable(self):
        """Single unconstrained external Int compared to literal — should be Intractable."""
        sensor = Int("Sensor", external=True)
        alarm = Bool("Alarm")
...
# ===================================================================
# Gap 3: Integer overflow / wraparound in threshold absorption
# ===================================================================


class TestGap3IntegerWraparound:
    """Int-progress absorption fast-forward must respect PLC integer wrapping.

    calc(C + 100, C) on an Int tag wraps at 32767.  If the fast-forward
    uses Python arithmetic, it overshoots and never reaches the negative
    range.  Timers and counters are NOT affected — they stop accumulating
    at preset, so overflow is impossible.  Only calc-based int progress
    tags (``_PROGRESS_KIND_INT_UP`` / ``_PROGRESS_KIND_INT_DOWN``) are
    vulnerable.
    """
...
# ===================================================================
# Gap 4: Concrete elision two-value check insufficient for non-Bool
# ===================================================================


class TestGap4ElisionNonBool:
    """Concrete elision must cover non-Bool domain paths.

    The concern is that a tag is read-before-write only under a specific
    numeric guard.  The implementation uses full domain enumeration (up to
    200K combos), not just two values, so this should already be handled.
    These tests verify the elision correctness for non-Bool scenarios.
    """
    ...
    def test_mode_gated_cross_scan_value_matters(self):
        ...
        stateful = result[0]
        assert "Temp" in stateful, "Temp must be stateful — it is read before written under Mode=7"

    def test_conditional_read_different_guard_values(self):
        """Tag read-before-write under one guard value, write-before-read under another."""
...
# ===================================================================
# Gap 5: Fast-forward assumes stable enabling during skipped scans
# ===================================================================


class TestGap5InputInterleavingDuringFastForward:
    """Inputs that latch during the timer accumulation window must compose
    with the timer-done state.

    The BFS three-valued abstraction (False/PENDING/True) collapses all
    intermediate ACC values.  All input combos are explored from the
    PENDING state, so rise(Trigger) -> Latch=True produces a new state
    (PENDING, Latch=True) from which the event jump fires.
    """
    ...
    def test_timer_plus_counter_interleaving(self):
        """Timer done + counter done from independent inputs — both reachable simultaneously."""
...
# ===================================================================
# Gap 6: Real-valued tags as BFS state keys
# ===================================================================


class TestGap6RealFloatStateKeys:
    """Real (float) tags with feedback loops must not cause non-termination.

    The BFS should either absorb them into threshold vectors or declare
    Intractable — not loop forever.
    """
...
# ===================================================================
# Gap 7: Edge compression with state-dependent determination
# ===================================================================


class TestGap7MultiWriteEdgeCompression:
    """Multiple rungs writing the same output (priority logic) must not
    be collapsed by edge compression.

    Edge compression is about rise()/fall() input prev values, not
    about output write conflicts.  This test confirms the BFS correctly
    explores both writers.
    """

    def test_two_latching_writers_same_tag(self):
        """Two rungs latch the same tag from different conditions — both paths explored."""
...
# ===================================================================
# Gap 8: Threshold absorption with conditional resets
# ===================================================================


class TestGap8ConditionalResets:
    """Self-resetting counters (modular counters, sawtooth patterns) must
    not be hidden by threshold absorption.

    calc(C + 1, C) with GEQ(C, 50) -> copy(0, C) creates a sawtooth.
    The absorption must detect the reset and explore post-reset states.
    """
