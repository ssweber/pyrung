"""Crossings Phase 2 — CalcCrossing (affine equality inversion)."""
...
def test_affine_add_inverts_to_preimage() -> None:
    ...
    r = _CALC.reverse(CalcInstruction(src + 5, dest), "Dest", 42, _ctx(src, dest))
    assert r.constraints == [("Src", frozenset({37}))]
...
def test_affine_negate_inverts() -> None:
    src, dest = Int("Src"), Int("Dest")
    r = _CALC.reverse(CalcInstruction(-src, dest), "Dest", 7, _ctx(src, dest))
    assert r.constraints == [("Src", frozenset({-7}))]


def test_affine_mul_exact_division_inverts() -> None:
    src, dest = Int("Src"), Int("Dest")
    r = _CALC.reverse(CalcInstruction(src * 3, dest), "Dest", 9, _ctx(src, dest))
    assert r.constraints == [("Src", frozenset({3}))]


def test_mul_non_integer_preimage_falls_through() -> None:
    src, dest = Int("Src"), Int("Dest")
    assert _CALC.reverse(CalcInstruction(src * 3, dest), "Dest", 7, _ctx(src, dest)).fallthrough
...
def test_sum_expr_falls_through() -> None:
    ...
    assert _CALC.reverse(instr, "Total", 1, _ctx(dest)).fallthrough is True


def test_multi_tag_expr_falls_through() -> None:
    a, b, dest = Int("A"), Int("B"), Int("Dest")
    assert _CALC.reverse(CalcInstruction(a + b, dest), "Dest", 5, _ctx(a, b, dest)).fallthrough


def test_non_numeric_target_falls_through() -> None:
    src, dest = Int("Src"), Int("Dest")
    assert _CALC.reverse(CalcInstruction(src + 5, dest), "Dest", "x", _ctx(src, dest)).fallthrough
