...
class TestDimensionClassification:
    def test_ote_only_all_combinational(self):
        ...
        stateful, nd, combinational, _done_acc = result
    ...
    def test_external_tags_are_nondeterministic(self):
        ...
        result = _classify_dimensions(logic)
        assert not isinstance(result, Intractable)
        _stateful, nd, _combinational, _done_acc = result
        assert "Sensor" in nd
    ...
    def test_readonly_tags_excluded(self):
        ...
        result = _classify_dimensions(logic)
        assert not isinstance(result, Intractable)
        stateful, nd, _combinational, _done_acc = result
    ...
    def test_timer_done_is_bool_state_dim(self):
        ...
        result = _classify_dimensions(logic)
        assert not isinstance(result, Intractable)
        stateful, _nd, _combinational, _done_acc = result
...
class TestDontCarePruning:
    def test_masked_input_not_live(self):
        ...
        state_expr = And((Atom("StateBit", "xic"), Atom("Input", "xic")))
    ...
    def test_live_inputs_partial_masking(self):
        ...
        exprs = [
            And((Atom("StateBit", "xic"), Atom("InputA", "xic"))),
            Atom("InputB", "xic"),
        ]
        nd_dims = {
            "InputA": (False, True),
    ...
    def test_all_inputs_live_in_or(self):
        ...
        exprs = [Or((Atom("InputA", "xic"), Atom("InputB", "xic")))]
...
class TestVerifyInvariant:
    def test_basic_invariant_holds(self):
    ...
    def test_invariant_violation(self):
        """Invariant that doesn't hold returns Counterexample."""
        button = Bool("Button", external=True)
        flag = Bool("Flag")
