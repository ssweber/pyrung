"""Delayed-tide reachability for a non-PackML, config-channeled program.

A "tool changer" whose enablement is a bitwise allow-mask indexed by a channel
register — the same *math* as the PackML disabled-state mask, but with zero
state-machine shape:

    ToolSel   (channel)     selects the active tool set
...
tool 3 is only allowed in config 2, so PILOT must change the channel first,
let the gauge re-read, then command the tool.

Three channel shapes, all valid user code, exercise three generalizations:

* **retained** — ``copy(ToolSelCmd, ToolSel)`` gated by ``!= 0``.  The channel
  ...
  is a plain copy, not a literal decode (``tide_tables._producible_int_domain``).

* **decoded** — ``rung(ToolSelCmd == 1): copy(1, ToolSel)`` …  The channel is a
  ...
  channel (it does not enumerate inputs like BFS does), so its macro-state key
  must keep the elided channel or the establish move reads as SPIN.

* **ext_index** — no channel register at all: the gauge is indexed directly by
...
def _tool_changer(*, channel: str):
    """Build a config-channeled tool-changer program; return ``(logic, ToolActive)``.

    *channel* is ``"retained"``, ``"decoded"``, or ``"ext_index"``.
    """
    ...
    cfg_choices = (
        {1: "SetA", 2: "SetB"} if channel == "decoded" else {0: "None", 1: "SetA", 2: "SetB"}
    )
    ...
    with Program(strict=False) as logic:
        ...
        if channel == "retained":
            # Channel retains its value when the command rests at 0 -> genuine
            # cross-scan state (mirrors packml UnitModeCmd -> UnitModeCurrent).
            with rung(ToolSelCmd != 0):
                copy(ToolSelCmd, ToolSel)
        elif channel == "decoded":
            # Channel decoded: always rewritten when the command is valid, so the
        ...
        with rung():
            ...
            # otherwise from the channel register.
            calc(200 + (ToolSelCmd if channel == "ext_index" else ToolSel), CfgIdx)
        with rung():
...
def test_delayed_tide_retained_channel():
    """Retained channel: the inverter enumerates ToolSel over the command's
    choices (plain copy, no literal decode), so the mode change is discovered."""
    logic, ToolActive = _tool_changer(channel="retained")
...
def test_delayed_tide_decoded_channel_survives_elision():
    """Decoded channel: the prover elides ToolSel as scan-local, but the pilot's
    pre-elision macro-state key keeps it, so the establish move is not SPIN."""
    logic, ToolActive = _tool_changer(channel="decoded")
    path = _reach(logic, ToolActive)
    assert path.reachable, f"unreachable: {path.reason}"
    assert path.replay().state.tags["ToolActive"] == 3
...
def test_delayed_tide_command_indexed_gauge():
    """No channel register: the gauge pointer is computed from the command
    ...
    logic, ToolActive = _tool_changer(channel="ext_index")
    path = _reach(logic, ToolActive)
    assert path.reachable, f"unreachable: {path.reason}"
    assert path.replay().state.tags["ToolActive"] == 3
