...
def test_unrepresentable_regression_fails_closed_without_installing_logic(monkeypatch):
    """Evidence that cannot identify one exact correction stays uninstalled."""
    ...
    assert tuple(state.pilot_rungs) == ()
    assert regression.data["investigation"]["legacy_replay"] is False
    assert regression.data["investigation"]["working_theory_admission"] == "unrepresentable"
    assert regression.data["revoked_pilot_rungs"] == ()
