...
def test_codegen_emits_slot_for_indirect_only_nicknamed_address(tmp_path: Path):
    ...
    # Slot-carried identity, no standalone Tag, no TagMap entry
    assert 'ds.slot(165, name="sm__JUMPRESETTING2IDLE", default=4)' in code
    assert "sm__JUMPRESETTING2IDLE = ds[165]" in code
    assert 'Int("sm__JUMPRESETTING2IDLE"' not in code
    assert "sm__JUMPRESETTING2IDLE.map_to" not in code
    assert "sm__JUMPRESETTING2IDLE:" not in code
