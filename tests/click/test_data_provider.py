...
def test_read_mapped_standalone_tag_returns_runner_state_value():
    ...
    runner = PLC(logic=[], initial_state=SystemState().with_tags({"Valve": True}))
...
def test_write_mapped_tag_is_deferred_until_next_scan():
    valve = Tag("Valve", TagType.BOOL)
    mapping = TagMap({valve: c[1]})
    runner = PLC(logic=[], initial_state=SystemState().with_tags({"Valve": False}))
    provider = ClickDataProvider(runner, mapping)
...
def test_unmapped_addresses_delegate_to_fallback_provider():
    ...
    provider = ClickDataProvider(PLC(logic=[]), TagMap({valve: c[1]}), fallback=fallback)
...
def test_xd0_reflects_x001_to_x016_bit_image():
    provider = ClickDataProvider(PLC(logic=[]), TagMap())
...
def test_yd_write_can_be_overwritten_by_rung_outputs_in_same_scan():
    ...
    runner = PLC(logic=logic, initial_state=SystemState().with_tags({"Drive": False}))
    provider = ClickDataProvider(runner, mapping)
...
def test_mapped_read_uses_slot_or_tag_default_when_absent():
    ...
    provider = ClickDataProvider(PLC(logic=[]), mapping)
