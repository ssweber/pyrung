...
_PLC_DIR = "src/plc"
_MAIN_PATH = f"{_PLC_DIR}/main.py"
_TAGS_PATH = f"{_PLC_DIR}/tags.py"
_SUBROUTINES_DIR = f"{_PLC_DIR}/subroutines"
...
def _exec_project(files: dict[str, str], tmp_path: Path) -> dict:
    ...
    try:
        tags_mod = importlib.import_module("plc.tags")
        added_modules.append("plc.tags")

        # Import subroutine modules
        for rel_path in sorted(files):
            if (
                rel_path.startswith(f"{_SUBROUTINES_DIR}/")
                and rel_path.endswith(".py")
                and rel_path != f"{_SUBROUTINES_DIR}/__init__.py"
            ):
                mod_name = rel_path.removeprefix("src/")[:-3].replace("/", ".")
                importlib.import_module(mod_name)
                added_modules.append(mod_name)

        # subroutines package itself
        if f"{_SUBROUTINES_DIR}/__init__.py" in files:
            if "plc.subroutines" not in sys.modules:
                importlib.import_module("plc.subroutines")
            added_modules.append("plc.subroutines")

        main_mod = importlib.import_module("plc.main")
        added_modules.extend(("plc.main", "plc"))
...
class TestProjectBasic:
    ...
    def test_release_scaffolding_and_experimental_how_guidance(self, tmp_path: Path):
        ...
        assert '"pyrung>=0.14.0"' in files["pyproject.toml"]
    ...
    def test_tags_no_inline_comments(self, tmp_path: Path):
        ...
        # Nicknames emit as slot aliases (suppress_comments strips # X001)
    ...
    def test_plain_block_range_project_imports_block_symbol(self, tmp_path: Path):
        ...
        assert 'CmdTagBits = Block("CmdTagBits", TagType.BOOL, 1, 19)' in files[_TAGS_PATH]
        assert "Cmd_Mode_Production = CmdTagBits[4]" in files[_TAGS_PATH]
        assert "from .tags import mapping" in files[_MAIN_PATH]
        assert "Cmd_Mode_Production" in files[_MAIN_PATH]
        assert "CmdTagBits" in files[_MAIN_PATH]
        assert "out(Cmd_Mode_Production)" in files[_MAIN_PATH]
        assert "reset(CmdTagBits.select(4, 6))" in files[_MAIN_PATH]
        assert "C1004_to_C1006" not in files[_TAGS_PATH]
        assert "C1004_to_C1006" not in files[_MAIN_PATH]
    ...
    def test_plain_block_project_lifts_uniform_retentive_to_block_default(self, tmp_path: Path):
        ...
        assert (
            'AlarmCoil = Block("AlarmCoil", TagType.BOOL, 1, 3, retentive=True)'
            in files[_TAGS_PATH]
        )
        assert "AlarmCoil.slot(1, name='AlarmCoil_PLC')" in files[_TAGS_PATH]
        assert "AlarmCoil.slot(2, name='AlarmCoil_LostData')" in files[_TAGS_PATH]
        assert "AlarmCoil.slot(3, name='AlarmCoil_Watchdog')" in files[_TAGS_PATH]
        assert (
            "retentive=True"
            not in files[_TAGS_PATH].split(
...
class TestSubroutineCallsSubroutine:
    ...
    def test_cross_sub_import(self, tmp_path: Path):
        """sub_a calls sub_b — sub_a.py uses a package-relative import."""
        from pyrung.click.codegen.collector import _collect_operands
        from pyrung.click.codegen.models import (
            _AnalyzedRung,
            _InstructionInfo,
...
class TestDiskWrite:
    ...
    def test_output_dir(self, tmp_path: Path):
        ...
        main_path = out_dir / _MAIN_PATH
        tags_path = out_dir / _TAGS_PATH
        assert main_path.exists()
        assert tags_path.exists()
        assert main_path.read_text(encoding="utf-8") == files[_MAIN_PATH]
