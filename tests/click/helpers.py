...
def strip_pyrung_boilerplate(code: str) -> str:
    """Remove import boilerplate from generated pyrung code.

    Strips ``# Generated by pyrung`` headers, ``from pyrung …`` import lines,
    and leading blank lines.  Returns everything from the first tag declaration
    or ``with Program`` block onward, normalized.

    Raises ``ValueError`` if no ``with Program`` block is found.
    """
    lines = code.splitlines()

    # Find the first line that isn't a comment, import, or blank
    body_start = None
    ...
    if body_start is None:
        raise ValueError("No body found after stripping boilerplate")

    body = "\n".join(lines[body_start:])

    if "with Program" not in body:
        raise ValueError(
            f"Expected 'with Program' in body after stripping boilerplate, "
            f"got:\n{body[:200]}"
        )

    return normalize_pyrung(body)
